...
interface EquityRatioChartProps {
  data: FinancialData[];
}

export default function EquityRatioChart({ data }: EquityRatioChartProps) {
  const chartData = [...data].reverse().map((item) => ({
    year: `${item.fiscal_year}年`,
    equity_ratio: item.metrics?.equity_ratio || null,
  }));

  const getHealthColor = (value: number | null) => {
    if (value === null) return chartColors.grid;
    if (value >= 40) return chartColors.healthy;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">自己資本比率推移</h3>
        ...
        <LineChart data={chartData}>
          ...
          <Line
            ...
            dot={(props: any) => {
              ...
              const color = getHealthColor(payload.equity_ratio);
              return (
                <circle
