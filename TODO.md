...
### 2.1 銘柄検索機能
- [ ] **バックエンド**: 銘柄検索API実装
  - [ ] `GET /api/companies/search` エンドポイント作成
  - [ ] 銘柄コード検索ロジック
  - [ ] 銘柄名部分一致検索ロジック
  - [ ] レスポンススキーマ定義
- [ ] **バックエンド**: 企業マスタデータ取得
  - [ ] バフェット・コードAPIクライアント作成
  - [ ] 企業情報取得関数
  - [ ] DBへのデータ保存ロジック
...
### 2.2 企業情報表示
- [ ] **バックエンド**: 企業詳細API実装
  - [ ] `GET /api/companies/{stock_code}` エンドポイント
  - [ ] 企業概要取得
  - [ ] 事業内容取得
  - [ ] アナリスト評価取得（可能な場合）
- [ ] **バックエンド**: 最新ニュース取得
...
### 2.3 株価データ取得・表示
- [ ] **バックエンド**: 株価データ取得API
  - [ ] `GET /api/companies/{stock_code}/stock-prices` エンドポイント
  - [ ] yfinanceクライアント作成 (`services/yfinance_client.py`)
  - [ ] 株価データ取得関数
  - [ ] 期間指定対応 (1週間、1ヶ月、3ヶ月、6ヶ月、1年、5年)
  - [ ] DBへのキャッシュ保存
  - [ ] 15分キャッシュロジック
- [ ] **フロントエンド**: 株価チャートコンポーネント
  - [ ] `components/StockChart.tsx`
  - [ ] Rechartsでローソク足チャート実装
  - [ ] 移動平均線表示 (25日、75日、200日)
  - [ ] 出来高グラフ
  - [ ] 期間選択UI
  - [ ] ズーム・パン機能
